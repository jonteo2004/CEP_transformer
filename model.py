...
def scaled_dot_product_attention(query, key, value, mask):
    ...
    # So that large negative inputs to softmax are near zero in the output.
...
# Instead of one single attention head, query, key, and value are split into multiple heads
# because it allows the model to jointly attend to information at different positions

class MultiHeadAttention(tf.keras.layers.Layer):

    # __init__ mainly gets data from Hparams
    ...
    # reshaping tensors to split it into the heads
    def split_heads(self, inputs, batch_size):
        inputs = tf.reshape(
            inputs, shape=(batch_size, -1, self.num_heads, self.depth))
        return tf.transpose(inputs, perm=[0, 2, 1, 3])

    #Making the Multi-Head attention layer
...
# Instead of pure Encoding, where words with similar meaning have similar encodings, this Positional Encoding has both
# a) Closer to each other based on the similarity of their meaning
# b) Their position in the sentence, in the d-dimensional space.
...
# a) Multi-head attention (with padding mask)
# b) 2 dense layers followed by dropout
...
def encoder_layer(hparams, name="encoder_layer"):
    ...
    # implementing dropout (prevent overfit)
    ...
    # Applies a transformation that maintains the mean activation within each example close to 0 and the activation
...
# a) Masked multi-head attention (with look ahead mask and padding mask)
# b) Multi-head attention (with padding mask).
#   1. value and key receive the encoder output as inputs.
#   2. query receives the output from the masked multi-head attention sublayer.
...
def decoder_layer(hparams, name="decoder_layer"):
    ...
    attention1 = tf.keras.layers.LayerNormalization(epsilon=1e-6)(attention1 +
                                                                  inputs)
    # normal multi-head attention with encoder key and value from encoder outputs
